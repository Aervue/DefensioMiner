...
impl Rom {
    ...
    #[inline(always)]
    pub(crate) fn prefetch_line(&self, idx: u32) {
        #[inline(always)]
        unsafe fn prefetch(addr: *const u8) {
            #[cfg(target_arch = "aarch64")]
            unsafe {
                core::arch::asm!("prfm pldl1keep, [{0}]", in(reg) addr);
            }
        ...
        let nb_chunks = self.data.len() / DATASET_ACCESS_SIZE;
        if nb_chunks == 0 {
            return;
        }
        let chunk_index = (idx as usize) % nb_chunks;
        let offset = chunk_index * DATASET_ACCESS_SIZE;
        unsafe {
            prefetch(self.data.as_ptr().add(offset));
        }
    }
